<div class="">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                {{ $log->acao === 'Criar' ? 'bg-green-100 text-green-800' : '' }}
                                {{ $log->acao === 'Atualizar' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                {{ $log->acao === 'Eliminar' ? 'bg-red-100 text-red-800' : '' }}
                                {{ $log->acao === 'Visualizar' ? 'bg-gray-100 text-gray-800' : '' }}
                                {{ !in_array($log->acao, ['Criar', 'Atualizar', 'Eliminar', 'Visualizar']) ? 'bg-purple-100 text-purple-800' : '' }}
                            ">
                                {{ $log->acao }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 text-center">
                            @if($log->descricao)
                                <div title="{{ $log->descricao }}">
                                    {{ Str::limit($log->descricao, 60) }}
                                </div>
                            @else
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 text-center">
                            @if($log->user_agent)
                                <div title="{{ $log->user_agent }}" class="text-xs">
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="stat bg-base-200 rounded-lg p-4">
            ...
            <div class="stat-value text-secondary">{{ $logs->currentPage() }}</div>
        </div>
        <div class="stat bg-base-200 rounded-lg p-4">
            <div class="stat-title">Total de Páginas</div>
            <div class="stat-value">{{ $logs->lastPage() }}</div>
        </div>
        <div class="stat bg-base-200 rounded-lg p-4">
            <div class="stat-title">Logs por Página</div>
            <div class="stat-value">{{ $logs->perPage() }}</div>
        </div>
    </div>
</div>
